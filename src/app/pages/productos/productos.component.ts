...
export class ProductosComponent implements OnInit {
      ...
      this.productoService.agregarProducto(Producto).subscribe((data) => {
        if (data[0].id) {
          ...
          this.serviceCategoria
            .agregarCategoriaDetalle(CategoDetalle)
            .subscribe((data) => {
              if (data[0].msg) {
  ...
  guardarEdicion() {
    ...
    this.productoService.editarProducto(Producto).subscribe((data) => {
      if (data[0].msg) {
        ...
        this.notificaciones.onSuccess(
          'La edicion se completo satisfactoriamente',
          ''
        );
        this.Id_Edit = 0;
  ...
  agregarCategoria(id: number) {
    ...
    const Producto: ProductoModel = { Id_Producto: id };
    this.productoService
      .listarProductosVendedorCategoria(Producto)
      .subscribe((data) => {
  ...
  eliminarCategoProd(id: number) {
    ...
    this.serviceCategoria
      .eliminarCategoriaDetalle(CategoDetalle)
      .subscribe((data) => {
        if (data[0].msg) {
  ...
  agregarMultimedia(id: number) {
    this.Agregar_Editar = 'SubirMultimedia';
    this.Id_Edit = id;
  }
